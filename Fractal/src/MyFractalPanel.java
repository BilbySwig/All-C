...
public class MyFractalPanel extends FractalPanel{
	...
	private void RandomColorScheme(){
		...
		int r=0, g=0, b=0;//original numbers for color values, colors start at black and change from there
		int rchange=(int)(Math.random()*31), gchange=(int)(Math.random()*31), bchange=(int)(Math.random()*31);//increments of change for r, g, and b
	...
	private void englishColorScheme(){
		colorscheme=new ArrayList<Color>();
		colorscheme.add(new Color(44, 36, 22));
		colorscheme.add(new Color(186, 163, 120));
		colorscheme.add(new Color(69, 56, 35));
		colorscheme.add(new Color(192, 161, 114));
		colorscheme.add(new Color(56, 46, 28));
		colorscheme.add(new Color(186, 163, 120));
		
		int r=(int)(Math.random()*31), g=(int)(Math.random()*31), b=(int)(Math.random()*31);//increments of change for r, g, and b
		int rchange=(int)(Math.random()*31), gchange=(int)(Math.random()*31), bchange=(int)(Math.random()*31);//increments of change for r, g, and b
		for(int i=0;i<=256;i++){//loop to create bunches and bunches of colors
			colorscheme.add(new Color(r,g,b));//new color based on r, g, and b values
			r=(r+rchange)%255;//r,g,and b change
